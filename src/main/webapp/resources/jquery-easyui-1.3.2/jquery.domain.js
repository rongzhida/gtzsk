...
(function($) {
    ...
    function build(target) {
					...
					onSelect: function(record) {//适用于单选或多选combobox，以及单选或多选combotree
						if (c == "combobox") {
							...
							if (opts.multiple == false) {//适用于单选combotree
								...
								//box.combotree("clear");
								//box.combotree("tree").tree("check", record.target);
							}
						}
					},
    	...
    	//事件处理函数
		function eventHandler(target, event, fn) {
			if(!fn) {
				return;
			}
			var t = $(target);
			if ((t.attr("autoQuery") + ",true").toLowerCase().indexOf(event.toLowerCase()) != -1) {
        ...
        top.$.messager.confirm("确认", m, function(result) {
            if (result) {
                $.ajax({
                    ...
                    dataType: opts.dataType,
                    success: function(data, status, XHR) {
                    	m = $.format("删除 <font style='color:red'>{0}</font> 条{1}成功。", ids.length, title);
                        top.$.messager.show({ title: "信息", msg: m, timeout: 4000, showType: "slide" });
                        window.setTimeout(function() {
